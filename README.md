...
### 👶 初心者ガイド（推奨）
- [初めて Claude Code を使う人へ](#-初めて-claude-code-を使う人へ)
- [便利な使い方](#-便利な使い方) - **会話リセット、モード切り替え**
- [よくある失敗と対策](#-よくある失敗と対策)

### 📚 詳細情報（任意）
- [カスタマイズ方法](#-カスタマイズ方法)
- [プロジェクト用のルール設定](#-プロジェクト用のルール設定)
- [トラブルシューティング](#-トラブルシューティング)
- [もっと詳しく知りたい](#-もっと詳しく知りたい)
...
2つの方法があります：
- `/clear` コマンドを使う
- Claude Code を終了して再起動（`exit` → `claude`）
...
**困ったときは:**
1. まず会話をリセットしてみる（`/clear` または 再起動）
2. Plan モードを使ってみる（Shift+Tab で切り替え）
3. このREADMEを読み返す
